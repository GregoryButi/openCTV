...
input_dir = "/home/gregory/Downloads/results_folder_edits_HAS_train"

# Set the output directory
output_dir = "/home/gregory/Downloads/results_folder_edits_HAS_valid_2"
...
for PID, i in zip(PIDs_valid, range(len(PIDs_valid))):
    ...
    BLD_CTV = compute_bidirectional_distance_volume(CTV_DL.getMeshpoints(), CTV.getMeshpoints(), CTV_DL.gridSize,
    ...
    for j, mask in enumerate(CTV_DL.preferred['masks']):
        distance_barrier_soft[i, j] = CTV_DL.distance3D[mask.imageArray].min()
        overlap = np.logical_and(mask.imageArray, CTV_DL.imageArray)
        if np.any(overlap):
            margin_reduction_soft[i, j] = (CTV_DL.isodistance - CTV_DL.distance3D[overlap].max())
        else:
