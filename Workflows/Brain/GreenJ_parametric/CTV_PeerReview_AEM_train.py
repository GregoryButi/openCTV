...
from Process.Analysis.contourComparison import dice_score, percentile_hausdorff_distance, surface_dice_score, compute_bidirectional_distance_volume
...
# patient IDs
PIDs = ['TVD_025_GBM', 'TVD_026_GBM', 'TVD_027_GBM', 'TVD_028_GBM', 'TVD_029_GBM', 'TVD_030_GBM', 'TVD_031_GBM', 'TVD_032_GBM', 'TVD_033_GBM', 'TVD_034_GBM', 'TVD_035_GBM', 'TVD_036_GBM', 'TVD_037_GBM', 'TVD_038_GBM', 'TVD_039_GBM', 'TVD_040_GBM', 'TVD_042_GBM', 'TVD_043_GBM', 'TVD_044_GBM', 'TVD_045_GBM',
        'TVD_046_GBM', 'TVD_047_GBM', 'TVD_048_GBM', 'TVD_049_GBM', 'TVD_050_GBM', 'TVD_051_GBM', 'TVD_052_GBM', 'TVD_053_GBM', 'TVD_054_GBM', 'TVD_055_GBM', 'TVD_056_GBM', 'TVD_057_GBM', 'TVD_058_GBM', 'TVD_059_GBM', 'TVD_060_GBM', 'TVD_061_GBM', 'TVD_062_GBM', 'TVD_063_GBM', 'TVD_064_GBM', 'TVD_065_GBM',
        'TVD_066_GBM', 'TVD_067_GBM', 'TVD_068_GBM', 'TVD_069_GBM', 'TVD_070_GBM', 'TVD_071_GBM', 'TVD_072_GBM', 'TVD_073_GBM'] # Physician: AEM
Crop_GTV_with_ventricles = [True, True, False, True, True, True, True, False, True, True, True, False, True, False, True, True, False, False, False, True,
                            True, False, False, False, False, False, False, False, False, False, False, True, False, True, True, False, False, False, False, False,
                            False, False, False, True, True, True, True, False]

# Set the random seed for reproducibility
random.seed(42)

# Get 80% of the indices
num_train = int(0.8 * len(PIDs))
indices_train = random.sample(range(len(PIDs)), num_train)

# Use the indices to get elements from both lists
PIDs_train = [PIDs[i] for i in indices_train]
Crop_GTV_with_ventricles_train = [Crop_GTV_with_ventricles[i] for i in indices_train]

# Set the output directory
output_dir = "/home/gregory/Downloads/results_folder_edits_AEM_train"
os.makedirs(output_dir, exist_ok=True)

indices_all = set(range(len(PIDs)))
indices_valid = list(indices_all - set(indices_train))
PIDs_valid = [PIDs[i] for i in indices_valid]
Crop_GTV_with_ventricles_valid = [Crop_GTV_with_ventricles[i] for i in indices_valid]

# Save PIDs_train to a text file
with open(os.path.join(output_dir, "PIDs_training.txt"), "w") as f:
    for pid in PIDs_train:
        f.write(f"{pid}\n")

with open(os.path.join(output_dir, "Crop_GTV_with_ventricles_training.txt"), "w") as f:
    for crop in Crop_GTV_with_ventricles_train:
        f.write(f"{crop}\n")

# Save PIDs_valid to a text file
with open(os.path.join(output_dir, "PIDs_valid.txt"), "w") as f:
    for pid in PIDs_valid:
        f.write(f"{pid}\n")

with open(os.path.join(output_dir, "Crop_GTV_with_ventricles_valid.txt"), "w") as f:
    for crop in Crop_GTV_with_ventricles_valid:
        f.write(f"{crop}\n")
...
dice_PIDs = np.zeros((len(PIDs_train), ))
HD95_PIDs = np.zeros((len(PIDs_train), ))
SDS_PIDs = np.zeros((len(PIDs_train), len(SDS_tolerance)))
volume_diff_PIDs = np.zeros((len(PIDs_train), ))
isodistance_PIDs = np.zeros((len(PIDs_train), ))
barrier_movement_PIDs = np.zeros((len(PIDs_train), 2))
resistance_PIDs = np.zeros((len(PIDs_train), 2))
distance_barrier_movement = np.zeros((len(PIDs_train), 2))
distance_barrier_soft = np.zeros((len(PIDs_train), 2))
margin_reduction_soft = np.zeros((len(PIDs_train), 2))

for PID, i, Crop_GTV in zip(PIDs_train, range(len(PIDs_train)), Crop_GTV_with_ventricles_train):
    ...
    CTV_DL = CTVGeometric(rts=RTs, model=model, spacing=voxel_size, origin=CTV.origin)
    ...
    for j, mask in enumerate(CTV_DL.moving['masks']):
        ...
        CTV_tmp = CTVGeometric(rts=RTs_tmp, model={'model': 'Nonuniform', 'obstacle': True}, spacing=voxel_size)
    ...
    HD95_PIDs[i] = percentile_hausdorff_distance(CTV.getMeshpoints(), CTV_DL.getMeshpoints(), percentile=95, voxel_spacing=voxel_size)
    ...
    print(f"Margin: {CTV_DL.isodistance:.2f} mm")
    for j, mask in enumerate(CTV_DL.moving['masks']):
        print(f"{mask.name} movement: {barrier_movement_PIDs[i,j]:.2f} mm")
    print(f"Volume difference: {volume_diff_PIDs[i]*100:.2f}%")
    for j, mask in enumerate(CTV_DL.preferred['masks']):
        print(f"Resistance {mask.name}: {resistance_PIDs[i,j]:.2f}")
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"Margin: {CTV_DL.isodistance:.2f} mm\n")
        for j, mask in enumerate(CTV_DL.moving['masks']):
            f.write(f"{mask.name} movement: {barrier_movement_PIDs[i, j]:.2f} mm\n")
        for j, mask in enumerate(CTV_DL.preferred['masks']):
            f.write(f"Resistance {mask.name}: {resistance_PIDs[i,j]:.2f}\n")
...
with open(summary_file, "w") as f:
    ...
    # Barrier Movements
    for j, mask in enumerate(CTV_DL.moving['masks']):
        values = barrier_movement_PIDs[:, j]
        f.write(f"{mask.name} Movement Mean: {np.mean(values):.2f} mm\n")
        f.write(f"{mask.name} Movement Std: {np.std(values):.2f} mm\n\n")
    ...
    # Resistance
    for j, mask in enumerate(CTV_DL.preferred['masks']):
        values = resistance_PIDs[:, j]
        f.write(f"Resistance {mask.name} Mean: {np.mean(values):.2f}\n")
        f.write(f"Resistance {mask.name} Std: {np.std(values):.2f}\n")
