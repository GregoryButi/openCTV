...
input_dir = "/home/gregory/Documents/Projects/CTV_RO1/Results/PeerReview/results_folder_edits_HAS_train"

# Set the output directory
output_dir = "/home/gregory/Downloads/results_folder_edits_HAS_valid"
...
# load numpy arrays
isodistances_PIDs_train = np.load(os.path.join(input_dir, "isodistance_PIDs.npy"))
barrier_movement_PIDs_train = np.load(os.path.join(input_dir, "barrier_movement_PIDs.npy"))
resistance_PIDs_train = np.load(os.path.join(input_dir, "resistance_PIDs.npy"))
distance_barrier_movement_PIDs_train = np.load(os.path.join(input_dir, "distance_barrier_movement.npy"))
distance_barrier_soft_PIDs_train = np.load(os.path.join(input_dir, "distance_barrier_soft.npy"))

# select values
isodistance = np.median(isodistances_PIDs_train)
barrier_movements = np.zeros((2,))
barrier_movements[0] = np.median(barrier_movement_PIDs_train[distance_barrier_movement_PIDs_train[:, 0] <= isodistances_PIDs_train, 0], axis=0)
barrier_movements[1] = np.median(barrier_movement_PIDs_train[distance_barrier_movement_PIDs_train[:, 1] <= isodistances_PIDs_train, 1], axis=0)
resistances = np.zeros((2,))
resistances[0] = np.median(resistance_PIDs_train[distance_barrier_soft_PIDs_train[:, 0] <= isodistances_PIDs_train, 0], axis=0)
resistances[1] = np.median(resistance_PIDs_train[distance_barrier_soft_PIDs_train[:, 1] <= isodistances_PIDs_train, 1], axis=0)
...
for PID, i in zip(PIDs_valid, range(len(PIDs_valid))):
    ...
    CTV_DL.preferred['resistances'] = resistances
    CTV_DL.setCTV_isodistance_movingBarriers(isodistance, barrier_movements, solver='FMM', domain=GTVBox)
    ...
    directory = os.path.join(os.path.join(os.path.join(patient_dir, PID), 'Therapy-scan'),
                             'Targets_' + model_segmentation + '_' + config_segmentation)
    ...
    if interactive:
        ...
        def plot_isodistance_sagittal(ax, X, plotCTV, plotCTV_DL, plotROI=None):
            ...
            fig.add_axes(ax)
            plt.imshow(np.flip(plotCT[X, :, :].transpose(), axis=0), extent=[y[0], y[-1], z[0], z[-1]], cmap='gray',
                       vmin=-1000, vmax=600)
            ...
            plt.contour(plotCTV_DL[X, :, :].transpose(), extent=[y[0], y[-1], z[0], z[-1]], colors='red',
        ...
        def plot_isodistance_coronal(ax, Y, plotCTV, plotCTV_DL, plotROI=None):
            ...
            fig.add_axes(ax)
            plt.imshow(np.flip(plotCT[:, Y, :].transpose(), axis=0), extent=[x[0], x[-1], z[0], z[-1]], cmap='gray',
                       vmin=-1000, vmax=600)
            ...
            plt.contour(plotCTV_DL[:, Y, :].transpose(), extent=[x[0], x[-1], z[0], z[-1]], colors='red',
                        linewidths=1.5, linestyles='dashed')
        ...
        def plot_isodistance_axial(ax, Z, plotCTV, plotCTV_DL, plotROI=None):
            ...
            fig.add_axes(ax)
            plt.imshow(np.flip(plotCT[:, :, Z].transpose(), axis=0), extent=[x[0], x[-1], y[0], y[-1]], cmap='gray',
                       vmin=-1000, vmax=600)
            ...
            plt.contour(plotCTV_DL[:, :, Z].transpose(), extent=[x[0], x[-1], y[0], y[-1]], colors='red',
                        linewidths=1.5, linestyles='dashed')
...
for tau, j in zip(SDS_tolerance, range(len(SDS_tolerance))):
    print(
        f"CTV DL mean surface dice (\u03C4 = {tau} mm): {SDS_PIDs_mean[j] * 100:.2f}% +/- {SDS_PIDs_std[j] * 100:.2f}%")
